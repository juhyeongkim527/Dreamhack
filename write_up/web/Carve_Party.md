...
그리고 더 아래를 살펴보면 해당 문제에서 중요한 아래의 함수가 보인다.
...
`pumkin` 배열과 `pie` 변수의 값은 위의 `make()` 함수에서 `counter > 10000`인 경우, 참조하여 계산되는데 해당 배열의 값이 핸들러 코드의 루틴대로 정상적으로 세팅되야 플래그가 출력되는 것이구나라고 예측했다.
...
그럼, `$('#jack-target').click()`의 핸들러가 `10000`번 중복되어 등록되게 되어, 한번만 클릭을 해줘도 핸들러 코드가 `10000`번 수행되어 루틴이 한번만에 수행되었기 때문이다.

따라서 `console`에 입력해준 것만으로는 `counter`를 증가시키지 않지만, `click` 이벤트 핸들러 함수를 `10000`번 등록해주게 되어 한번만 클릭해줘도 핸들러 코드가 `10000`번 수행되어 루틴대로 순차적으로 수행 후 플래그가 출력되는 것이다.
