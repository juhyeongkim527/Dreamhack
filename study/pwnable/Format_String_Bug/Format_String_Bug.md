...
**나중에 살펴 보니, `secret`의 주소를 먼저 전달해주면 주소의 상위 `2바이트`는 `\x00\x00`이므로 널바이트가 된다. 따라서, `printf(buf)` 도 결국 문자열을 처리하기 때문에 널바이트를 만나면 출력을 종료하게 되어 `p64(addr_secret)` 부터 전달하면 중간에 `\x00\x00` 때문에 형식지정자 부분이 `buf`에만 저장되고 포맷스트링에는 적용이 안된다.**
