...
계속 헷갈리는데, 리틀엔디언에서 문자열을 입력할 경우 예를 들어 `"AAA B"` 를 입력하면 `MSB(가장 왼쪽)`인 `A`가 가장 낮은 주소에 들어가고 `LSB(가장 오른쪽)`인 `B`가 가장 높은 주소에 들어가기 때문에 8바이트로 만들 때, 뒤에 `b"\x00"`을 더해줘야 하는 것이다.

반대로, 정수를 입력할 때는 `0x1234`라는 값은 8바이트로 채워지지 않은 경우 `LSB(가장 오른쪽)`인 `0x34`가 가장 낮은 주소에 들어가고, 차례대로 `0x34`가 높은 주소로 채워져서 `0x0000..3412`가 채워진다고 생각하면 된다.

**정리하자면, 리틀엔디언에서 문자열로 읽을 때나 입력할 때는 낮은 주소부터 읽는다고 생각하면 되고, 정수로 해석할 때는 높은 주소부터 차례대로 해석하면 된다.**
